...
include("utils.jl")
include.(filter(contains(r".jl$"), readdir(joinpath(dirname(pathof(Asthma_Julia)),"modules/"); join=true)))
...
function process_demographic(simulation::Simulation,until_all_die=false)
    ...
    n_list = zeros(Int,simulation.time_horizon,2)
    event_list= ["antibiotic_exposure", "asthma_incidence", "asthma_prevalence", "death","alive",
    "control1","control2","control3","exacerbation","exacerbation_severity1",
    "exacerbation_severity2","exacerbation_severity3","emigration","immigration"]
    ...
    for cal_year in cal_years
        ...
        if cal_year != min_cal_year 
            num_new_born = round(Int, simulation.n * simulation.birth.estimate.N_relative[tmp_cal_year_index])
            num_immigrants = round(Int, num_new_born * sum(simulation.immigration.table[tmp_cal_year_index].n_prop_birth))
            # num_immigrants = 0
            # the total number of agents simulated in the cal year
    
            # for the first/initial year, we generate the initial population
            # otherwise we generate num_new_born + num_immigrants
            n_cal_year = num_new_born + num_immigrants

            new_born_indicator = vcat(trues(num_new_born),falses(num_immigrants))
            immigrants_index = sample(1:nrow(simulation.immigration.table[tmp_cal_year_index]),Weights(simulation.immigration.table[tmp_cal_year_index].weights),num_immigrants)
            immigrant_counter = 1
        end
        ...
        for i in 1:n_cal_year
            ...
            if cal_year == min_cal_year
                ...
                if new_born_indicator[i]
                    ...
                    # # immigrant
                    simulation.agent = process(Bool(simulation.immigration.table[tmp_cal_year_index].sex[immigrants_index[immigrant_counter]]),
                    simulation.immigration.table[tmp_cal_year_index].age[immigrants_index[immigrant_counter]],cal_year,tmp_cal_year_index)
                    event_matrix[simulation.agent.cal_year_index,simulation.agent.age+1,simulation.agent.sex+1,14] += 1
                    immigrant_counter += 1
                end
            end
            ...
            # but we really need a joint distribution
            if simulation.agent.age != 0

                if process(simulation.agent,simulation.death)
                    @set! simulation.agent.alive = false
                    # everyone dies in the end... Inevitable
                    event_matrix[simulation.agent.cal_year_index,simulation.agent.age+1,simulation.agent.sex+1,4] += 1
                # # emigration
                elseif process(simulation.agent.cal_year_index,simulation.agent.age,simulation.agent.sex,simulation.emigration)
                    @set! simulation.agent.alive = false
                    # event_matrix[simulation.agent.cal_year_index,simulation.agent.age+1,simulation.agent.sex+1,5] += 1
                    event_matrix[simulation.agent.cal_year_index,simulation.agent.age+1,simulation.agent.sex+1,13] += 1
                else
